 ...
 * @author JRY
 */
public class JsonToolV2_asiakasT {
...
//        }
        ...
        JSONValueAsiakas asiakasV = new JSONValueAsiakas(deli);
        ArrayList<String> aslista = new ArrayList();
        ...
        tiedot.add("Liikevaihto");
        tiedot.add("Liiketoiminnan tulos");
        tiedot.add("Henkilöstö");
        tiedot.add("Liikevoitto-%");
        tiedot.add("Omavaraisuus");
        String header = "File";
        for (int i = 0; i < vuodet.size(); i++) {
            for (int l = 0; l < tiedot.size(); l++) {
                header = header + deli + vuodet.get(i) + " " + tiedot.get(l);
            }
        }
        aslista.add(header);
//        System.out.println(aslista);
        ...
        for (int l = 0; l < asiakasLista.size(); l++) {
            if(!asiakasLista.get(l).getFileName().equals("desktop.ini")){
                
            

            String data = "";
            try {
                data = asiakasLista.get(l).getFileName();
            } catch (Exception e) {
            }
            try {
                asiakasV.getMap(asiakasLista.get(l).getFileCont());
            } catch (Exception e) {
            }
            for (int i = 0; i < vuodet.size(); i++) {
                try {
                    data = data + deli + asiakasV.data.get(vuodet.get(i)).toString();
                } catch (Exception e) {
                    data = data + "||||";
                }
        ...
        StringFileHandler fileen2 = new StringFileHandler();
        fileen2.writeCSV(locaOut + "\\Asiakastietodata.txt", aslista);
...
//        if (suodatus) {
//            fileen.writeCSV(locaOut + "\\listaus med asiakastieto2.txt", listaCSV);
//        } else {
